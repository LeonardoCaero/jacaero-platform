...
- **Time Tracker — fotos**: al registrar/editar horas se pueden adjuntar imágenes (hasta 6, 8MB c/u). Guardadas en disco (`UPLOADS_PATH`, volumen Docker `jacaero_uploads` — no en el bind mount del NAS de `DOCS_ROOT_PATH`), servidas por endpoint autenticado (`GET /time-entries/:id/photos/:filename`), no estáticas ni públicas. Se borran del disco al borrar la foto o la propia entrada.
