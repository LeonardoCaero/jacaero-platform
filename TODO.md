...
- **Previsualizar pedidos (2026-09-23)**: en `/papeleo/pedidos`, botón de ojo en cada tarjeta y "Ver PDF" en el detalle abren el PDF en un modal con iframe dentro de la app (antes `window.open(blob:)`, que en móvil acababa descargando). Los ojos de presupuesto/albarán/factura del detalle usan el mismo modal.
- **FACTURAR OK automático (2026-09-23)**: `syncFacturarOk()` (al arrancar + cada hora) busca en Enviados de Gmail los adjuntos `NNN ALBARÁN ...` y guarda en `EmailOrder.albaranSentAt` la fecha de envío del albarán vinculado (primer envío posterior a la llegada del pedido). A las 48h, si no hay factura, pone la etiqueta Gmail `FACTURAR OK` al correo del pedido (búsqueda `from:remitente subject:<nº pedido>` en Todos) y guarda `facturarOkAt`; al vincularse la factura la quita. ImapFlow manda las etiquetas como átomo sin comillas → la etiqueta se pasa ya entrecomillada (`'"FACTURAR OK"'`). La app muestra la fecha de envío en el detalle y un badge FACTURAR OK en la tarjeta. Solo funciona si el albarán ya está vinculado al pedido (Vincular / manual).
