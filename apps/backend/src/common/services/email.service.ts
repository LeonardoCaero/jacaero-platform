      ...
      <tr>
        <td align="center">
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;width:100%;background:#fafafa;border-radius:16px;overflow:hidden;">
            <tr>
              <td style="padding:32px 32px 0;text-align:center;">
                <img src="data:image/png;base64,${LOGO_BASE64}" width="56" height="56" alt="J.A. Caero" style="display:inline-block;" />
          ...
          <p style="max-width:480px;margin:16px 0 0;color:#6b7280;font-size:12px;">
            J.A. Caero &middot; este correo se envió automáticamente, no respondas a esta dirección.
          </p>
        </td>
      </tr>
    </table>
...
export function buildInvitationEmail(inviteUrl: string, roleName: string) {
  const html = layout(
    `Te han invitado a unirte a J.A. Caero como ${roleName}.`,
    `
      <p style="margin:0 0 4px;">Hola,</p>
      <p style="margin:0;">Te han invitado a unirte a la plataforma de <strong>J.A. Caero</strong> como <strong>${roleName}</strong>.</p>
      ${button(inviteUrl, "Activar mi cuenta")}
      <p style="margin:0;color:#6b7280;font-size:13px;">Este enlace caduca en ${env.INVITATION_EXPIRES_IN_DAYS} días. Si no esperabas esta invitación, puedes ignorar este correo.</p>
    `,
  );

  return { subject: "Invitación a J.A. Caero", html };
}
